...
/**
 * Gets the olders questions from a list, sorted by the asking date in ascending order.
 *
 * @param {Question[]} qlist - The list of questions to sort
 *
 * @returns {Question[]} - The sorted list of questions
 */
const sortQuestionsByOldest = (qlist: Question[]): Question[] =>
  qlist.sort((a, b) => a.askDateTime.getTime() - b.askDateTime.getTime());
...
export const getQuestionsByOrder = async (order: OrderType): Promise<Question[]> => {
  try {
    ...
    if (order === 'oldest') {
      return sortQuestionsByOldest(qlist);
    }
