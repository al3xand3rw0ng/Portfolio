...
const questionController = (socket: FakeSOSocket) => {
  ...
  const addQuestion = async (req: AddQuestionRequest, res: Response): Promise<void> => {
    ...
    try {
      ...
      const user = await UserModel.findOne({ username: question.askedBy }).populate('friends');
      if (user && user.friends) {
        const friendUsernames = user.friends;
        const content = `Your friend ${question.askedBy} just posted a new question - "${question.title}" !`;
