...
describe('POST /addComment', () => {
  ...
  it('should add a new comment to the answer', async () => {
    ...
    jest.spyOn(UserNotification, 'insertMany').mockResolvedValueOnce([
      new UserNotification({
        ...
        recipient: 'user2',
        sender: 'admin',
        content: 'You have a new comment on your answer.',
        type: 'question',
        questionId: validAid.toString(),
        isRead: false,
        createdAt: new Date(),
      }),
    ]);
  ...
  it('should add a new comment to the question', async () => {
    ...
    jest.spyOn(UserNotification, 'insertMany').mockResolvedValueOnce([
      new UserNotification({
        ...
        recipient: 'user1',
        sender: 'admin',
        content: 'You have a new comment on your question.',
        type: 'question',
        questionId: validQid.toString(),
        isRead: false,
        createdAt: new Date(),
      }),
    ]);
  ...
  it('should add a new comment to the question and notify the author', async () => {
    const validQid = new mongoose.Types.ObjectId();
    const validCid = new mongoose.Types.ObjectId();
    const mockReqBody = {
      id: validQid.toString(),
    ...
    const mockUser = {
      _id: 'dummyUserId',
      username: 'dummyUser',
      friends: [{ _id: 'friendId1', username: 'friend1' }],
    ...
    jest.spyOn(UserModel, 'findOne').mockReturnValue(mockQuery as Query<User, User>);
    ...
    jest.spyOn(UserNotification, 'insertMany').mockResolvedValueOnce([
      new UserNotification({
        ...
        recipient: 'user1',
        sender: 'dummyUserId',
        content: `Your friend ${mockComment.commentBy} just posted a new comment to the question, "${mockQuestion.title}" !`,
        type: 'question',
        questionId: validQid.toString(),
        isRead: false,
        createdAt: new Date(),
      ...
      new UserNotification({
        ...
        recipient: 'author',
        sender: 'dummyUserId',
        content: `${mockComment.commentBy} just commented on your question: "${mockQuestion.title}" !`,
        type: 'question',
        questionId: validQid.toString(),
        isRead: false,
        createdAt: new Date(),
    ...
    expect(UserNotification.insertMany).toHaveBeenCalledTimes(2);
  });

  it('should add a new comment to the answer and notify the author and friends', async () => {
    const validAid = new mongoose.Types.ObjectId();
    const validCid = new mongoose.Types.ObjectId();
    const validQid = new mongoose.Types.ObjectId();
    ...
    addCommentSpy.mockResolvedValueOnce({} as Answer);
    ...
    jest.spyOn(AnswerModel, 'findById').mockResolvedValueOnce(mockAnswer);
    jest.spyOn(UserNotification, 'insertMany').mockResolvedValueOnce([
      new UserNotification({
        _id: new mongoose.Types.ObjectId(),
        recipient: 'answerId',
        sender: 'dummyUserId',
        content: `Your friend ${mockComment.commentBy} just posted a new comment to an answer of the question, "${mockQuestion.title}" !`,
        type: 'question',
        questionId: validAid.toString(),
        isRead: false,
        createdAt: new Date(),
      ...
      new UserNotification({
        ...
        recipient: 'answerId',
        sender: 'dummyUserId',
        content: `${mockComment.commentBy} just commented on your answer to the question: "${mockQuestion.title}" !`,
        type: 'question',
        questionId: validQid.toString(),
        isRead: false,
        createdAt: new Date(),
...
describe('GET /getCommentbyAuthor', () => {
  afterEach(async () => {
    await mongoose.connection.close(); // Ensure the connection is properly closed
  });
  ...
  it('should return all comments by the author', async () => {
    const validCid = new mongoose.Types.ObjectId();
    const mockReqQuery = {
      commentBy: 'dummyUserId',
    };
    ...
    jest.spyOn(CommentModel, 'find').mockResolvedValueOnce([mockComment]);

    const response = await supertest(app).get(
      `/comment/getCommentByAuthor?commentBy=${mockReqQuery.commentBy}`,
    );
    ...
    expect(CommentModel.find).toHaveBeenCalledWith({ commentBy: mockReqQuery.commentBy });
  });

  it('should return 500 if commentBy query parameter is missing', async () => {
    const response = await supertest(app).get('/comment/getCommentByAuthor');

    expect(response.status).toBe(500);
    expect(response.text).toBe('Error when getting comments by author');
  });
});
