...
describe('application module', () => {
  ...
  describe('Question model', () => {
    ...
    describe('getQuestionsByOrder', () => {
      test('get oldest questions, oldest questions sorted by oldest date', async () => {
        ...
        const result = await getQuestionsByOrder('oldest');
    ...
    describe('saveQuestion', () => {
      ...
      test('saveQuestion should return an error when there is an issue saving the question', async () => {
        const mockQn = {
          title: 'New Question Title',
          text: 'New Question Text',
          tags: [tag1, tag2],
        ...
        jest.spyOn(QuestionModel, 'create').mockImplementationOnce(() => {
          throw new Error('Error when saving a question');
        });
        const result = (await saveQuestion(mockQn)) as Question;
  ...
  describe('Answer model', () => {
    describe('saveAnswer', () => {
      ...
      test('saveAnswer should return an error when there is an issue saving the answer', async () => {
        const mockAnswer = {
        ...
        jest.spyOn(AnswerModel, 'create').mockImplementationOnce(() => {
          throw new Error('Error when saving an answer');
        });
        const result = (await saveAnswer(mockAnswer)) as Answer;
  ...
  describe('saveUser', () => {
    test('saveUser should return the saved user', async () => {
      ...
      const result = (await saveUser(mockSaveUser)) as User;

      expect(result._id).toBeDefined();
      expect(result.picture).toEqual(mockSaveUser.picture);
      expect(result.username).toEqual(mockSaveUser.username);
      expect(result.firstName).toEqual(mockSaveUser.firstName);
      expect(result.lastName).toEqual(mockSaveUser.lastName);
      expect(result.biography).toEqual(mockSaveUser.biography);
      expect(result.comments).toEqual([]);
      expect(result.answers).toEqual([]);
      expect(result.questions).toEqual([]);
      expect(result.friends).toEqual([]);
      expect(result.requests).toEqual([]);
      expect(result.privacySettings).toEqual(mockSaveUser.privacySettings);
    });

    test('saveUser should return an error when there is an issue saving the user', async () => {
      const mockUser = {
        picture: 'https://example.com/picture.jpg',
        username: 'janedoe',
        firstName: 'Jane',
  ...
  describe('Comment model', () => {
    describe('saveComment', () => {
      ...
      test('saveComment should return an error when there is an issue saving the comment', async () => {
        const mockComment = {
          text: 'New Comment Text',
          commentBy: 'comment3_user',
          commentDateTime: new Date('2024-06-06'),
        };
        jest.spyOn(CommentModel, 'create').mockImplementationOnce(() => {
          throw new Error('Error when saving a comment');
        });
        const result = (await saveComment(mockComment)) as Comment;
