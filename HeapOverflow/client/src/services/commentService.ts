 ...
 * @param commentBy - The author of the comments
 * @throws Error Throws an error if the request fails or the response status is not 200.
 */
const getComment = async (commentBy: string): Promise<Comment[]> => {
  const res = await api.get(`${COMMENT_API_URL}/getCommentByAuthor?commentBy=${commentBy}`);
  if (res.status !== 200) {
