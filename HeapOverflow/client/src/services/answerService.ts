 ...
 *
 * @param ansBy - The author of the answers
 * @throws Error Throws an error if the request fails or the response status is not 200.
 */
const getAnswer = async (ansBy: string): Promise<Answer[]> => {
  const res = await api.get(`${ANSWER_API_URL}/getAnswerByAuthor?ansBy=${ansBy}`);
  if (res.status !== 200) {
